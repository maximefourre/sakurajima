...
poi.js      — landmarks authorés (kuromatsu, rocher, pas japonais) + stoneYAt
...
1. **`test/invariants.html`** (via serve.py) : la console doit finir par
   ...
   disjoints, plus celui des lucioles, puis lot 2 : pas japonais du grand
   étang (`stoneYAt` fidèle), hérons en eau peu profonde, libellules dans
   l'habitat bassin. **`ponds.attach()` est appelé TÔT**, juste
